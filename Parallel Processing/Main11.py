...
import importlib 
import simulation11
import Tree_Models11
import Plot_Models11
import pandas as pd
import numpy as np
        ...
        for scenario in scenarios:
            ...
            for Each_period in harvest_periods:
                ...
                if __name__ == "__main__":
                    ...
                    importlib.reload(simulation11)
                    importlib.reload(Tree_Models11)
                    importlib.reload(Plot_Models11)
                    ...
                    if not os.path.isdir(jobDirectory):
                        ...
                        sys.exit(1)
                    start = time.time()        
                    prob = simulation11.Data(jobDirectory)
                    prob2= copy.deepcopy(prob)
                    if len(prob2.plots[x].treeList) != 0:
                        
                        plot = Plot_Models11.Species(Data= prob2, year = prob2.plots[x].year,plot_id = prob2.plots[x].plot_id, t=prob2.plots[x].stand_age_years, H40 =prob2.plots[x].SI_m, N=prob2.plots[x].N_tree_ha,
                        ...
                        simrun = simulation11.Simulator(plot = plot,jobDir= jobDirectory, scenario = scenario, Harvest_age = Each_period , MIC_TYPE = MIC_type, unique = uniqueNO,  YearN = True, Data= prob2)
                        ...
                        with open(current_working_directory(directory) +'/Tree_Simulated_Data/'+ str(x) +"_scenario" + str(scenario) + str(uniqueNO) + '_tree_simulation.csv', 'w', newline='') as csvfile:
                            ...
                            dict_keys = [{k: v.keys()} for k,v in Tree_Models11.GrowthModel.TITLES.items()]
                            
                            for t in dict_keys:
                                for i in t.keys():
                                    for periods in t.values():
                                        for j in periods:   
                                            writer.writerow({"plot_id": Tree_Models11.GrowthModel.TITLES[i][j].plot_id, 'tree_id': Tree_Models11.GrowthModel.TITLES[i][j].tree_id, 'tree_sp':Tree_Models11.GrowthModel.TITLES[i][j].tree_sp,'dbh':Tree_Models11.GrowthModel.TITLES[i][j].dbh, 
                                              'height':Tree_Models11.GrowthModel.TITLES[i][j].height,'diameter_class': Tree_Models11.GrowthModel.TITLES[i][j].diameter_class,'tree_Factor': Tree_Models11.GrowthModel.TITLES[i][j].tree_Factor,'SI_spp': Tree_Models11.GrowthModel.TITLES[i][j].SI_spp,
                                              'SI_m': Tree_Models11.GrowthModel.TITLES[i][j].SI_m , 'n_tree':Tree_Models11.GrowthModel.TITLES[i][j].n_tree,'species': Tree_Models11.GrowthModel.TITLES[i][j].species,'Period': Tree_Models11.GrowthModel.TITLES[i][j].Period,'coord_x': Tree_Models11.GrowthModel.TITLES[i][j].coord_x,
                                              'coord_y': Tree_Models11.GrowthModel.TITLES[i][j].coord_y, 'year': Tree_Models11.GrowthModel.TITLES[i][j].year, 'volsum': Tree_Models11.GrowthModel.TITLES[i][j].volsum, 'vol_spruce': Tree_Models11.GrowthModel.TITLES[i][j].vol_spruce, 'vol_pine': Tree_Models11.GrowthModel.TITLES[i][j].vol_pine,
                                              'vol_birch': Tree_Models11.GrowthModel.TITLES[i][j].vol_birch, 'vol_others': Tree_Models11.GrowthModel.TITLES[i][j].vol_others, 'vol_ROS': Tree_Models11.GrowthModel.TITLES[i][j].vol_ROS, 'vol_warm': Tree_Models11.GrowthModel.TITLES[i][j].vol_warm, 'management':Tree_Models11.GrowthModel.TITLES[i][j].management})   
